...
public class ArenaUtils {
	...
	public static void saveArenas() {
		for(Arena a : SpeedUHC.getInstance().getArenaManager().getArenas()) {
			File f = new File(SpeedUHC.getInstance().getDataFolder() + "/arenas/" + a.getName() + ".yml");
			if(!f.exists()) {
				try {
					f.createNewFile();
				} catch (IOException e) {
	...
	public static void loadArenas() {
		if(!new File (SpeedUHC.getInstance().getDataFolder() + "/arenas").exists())
			return;
		for(File fe : new File(SpeedUHC.getInstance().getDataFolder() + "/arenas").listFiles()) {
			FileConfiguration fc = new YamlConfiguration();
			try {
				fc.load(fe);
			} catch (IOException | InvalidConfigurationException e) {
			...
			File f = new File(SpeedUHC.getInstance().getDataFolder() + "/worlds/" + fc.getString("World"));

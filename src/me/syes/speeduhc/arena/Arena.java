...
public class Arena {
	...
	public Arena(Location corner1, Location corner2, String name) {
		...
		SpeedUHC.getInstance().getArenaManager().addArena(this);
	}
	...
	public void resetArena() {
		for(Player p : this.getArenaWorld().getPlayers()) {
			...
			if(p.getWorld() != SpeedUHC.lobbySpawn.getWorld())
		...
		new BukkitRunnable() {
			public void run() {
				...
				File f = new File(SpeedUHC.getInstance().getDataFolder() + "/worlds/" + name);
		...
		}.runTaskLater(SpeedUHC.getInstance(), 3);
	}
